...
class Slime_Game(arcade.Window):
    ...
    def setup_level_one(self):
        ...
        #increase Difficulty for levels
        x_count = 7
        x_start = 380
        x_spacing = 60
        y_count = 5
    ...
    def on_draw(self):
        ...
        # Render the text
        #add level system below
        arcade.draw_text(f"Score: {self.score}", 10, 20, arcade.color.WHITE, 20)
        arcade.draw_text(f"Level: {self.level}", 13, 50, arcade.color.WHITE, 20)
    ...
    def on_key_press(self, symbol: int, modifiers: int):
        ...
        if symbol == arcade.key.A or symbol == arcade.key.LEFT:
            self.player_sprite.change_x = -250
